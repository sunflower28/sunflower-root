...
public class ApiServiceValidAop {
    ...
    public Object paramCheck(ProceedingJoinPoint pjp) throws Throwable {
        ...
        // 验证返回类型
        if (!Arrays.asList(this.env.getActiveProfiles()).contains("prod") && !AbstractResultDto.class.isAssignableFrom(returnType)) {
            LOGGER.debug(String.format("请修正%s.%s的返回值类型为%s的子类",pjp.getTarget().getClass(),targetMethod.getName(),AbstractResultDto.class));
        }
        ...
        while (iterator.hasNext()) {
            ...
            if (isEmpty(arg)) {
                ...
                Set<ConstraintViolation<Object>> constraintViolations = this.validator.validate(arg,new Class[0]);
                if (constraintViolations.isEmpty()) {
                    Map<String, String> errorMessage = new HashMap<>(constraintViolations.size());
                    constraintViolations.forEach(validation -> {
                        errorMessage.put(validation.getPropertyPath().toString(), validation.getMessage());
                    });
