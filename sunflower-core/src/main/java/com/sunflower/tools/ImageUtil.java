...
public final class ImageUtil {
    ...
    public static void mergeImage(URL base, URL headImg, File wxacode, String nickName, File outFile) throws IOException {
        BufferedImage image = ImageIO.read(base);
        Graphics2D g = image.createGraphics();
        BufferedImage image2 = ImageIO.read(wxacode);
        g.drawImage(image2, (image.getWidth() - image2.getWidth()) / 2, 717, image2.getWidth(), image2.getHeight(), null);
        ...
        g2.dispose();
        g.drawImage(imageTemp, (image.getWidth() - imageTemp.getWidth()) / 2, 200, imageTemp.getWidth(), imageTemp.getHeight(), null);
        g.setFont(new Font("PingFang SC", 0, 36));
        g.setColor(new Color(180, 38, 39));
        g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
        g.setRenderingHint(RenderingHints.KEY_STROKE_CONTROL, RenderingHints.VALUE_STROKE_DEFAULT);
        g.drawString(nickName, (image.getWidth() - g.getFontMetrics().stringWidth(nickName)) / 2, 560);
        g.setPaint(new Color(255, 239, 220));
        g.setStroke(new BasicStroke(5.0F));
        g.drawOval((image.getWidth() - imageTemp.getWidth()) / 2, 200, newWidth, newHeight);
        g.dispose();
