...
/**
 * <p>
 * <b>Description:</b> RabbitMQ抽象消息监听，所有消息消费者必须继承此类
 * <p>
 * <b>Company:</b>
 *
...
public abstract class AbstractMessageListener implements ChannelAwareMessageListener {
	...
	public void onMessage(Message message, Channel channel) throws Exception {
		...
		Long consumerCount = redisTemplate.opsForHash().increment(
				MqConstants.MQ_CONSUMER_RETRY_COUNT_KEY, messageProperties.getMessageId(),
				1);

		log.info("收到消息,当前消息ID:{} 消费次数：{}", messageProperties.getMessageId(),
				consumerCount);
		...
		try {
			...
			if (consumerCount >= MqConstants.MAX_CONSUMER_COUNT) {
				...
				Thread.sleep(
						(long) (Math.pow(MqConstants.BASE_NUM, consumerCount) * 1000));
				redisTemplate.opsForHash().increment(
						MqConstants.MQ_CONSUMER_RETRY_COUNT_KEY,
