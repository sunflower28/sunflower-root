...
public class DbRedisCoordinator implements DbCoordinator {
	...
	public void setMsgReady(String msgId, RabbitMetaMessage rabbitMetaMessage) {
		this.redisTemplate.opsForHash().put(MqConstants.MQ_MSG_READY, msgId,
				rabbitMetaMessage);
	...
	public RabbitMetaMessage getMetaMsg(String msgId) {
		return (RabbitMetaMessage) this.redisTemplate.opsForHash()
				.get(MqConstants.MQ_MSG_READY, msgId);
	}
	...
	public List<RabbitMetaMessage> getMsgReady() throws Exception {
		HashOperations<String, String, RabbitMetaMessage> hashOperations = this.redisTemplateRabbitMetaMessage
				.opsForHash();
		List<RabbitMetaMessage> messages = hashOperations
				.values(MqConstants.MQ_MSG_READY);
	...
	public Long getResendValue(String key, String hashKey) {
		return (Long) this.redisTemplate.opsForHash().get(key, hashKey);
	}
	...
	private boolean messageTimeOut(String messageId) throws Exception {
		...
		long timeGap = System.currentTimeMillis()
				- new SimpleDateFormat(MqConstants.TIME_PATTERN).parse(messageTime)
						.getTime();
